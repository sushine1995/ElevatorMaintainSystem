...
<div id="pagination" class="box-footer clearfix">
	<ul class="pagination pagination-sm no-margin pull-right">
		 <% if (page.hasPrevious()){%>
               	<li><a href="${action}?pageNumber=1&pageSize=${paginationSize}&${searchParams}">首页</a></li>
                <li><a href="${action}?pageNumber=${current-1}&pageSize=${paginationSize}&${searchParams}">&laquo;</a></li>
         <%}else{%>
		...
		<c:forEach var="i" begin="${begin}" end="${end}">
            <c:choose>
                <c:when test="${i == current}">
                    <li class="active"><a href="?pageNumber=${i}&pageSize=${paginationSize}&${searchParams}">${i}</a></li>
                </c:when>
                <c:otherwise>
                    <li><a href="${action}?pageNumber=${i}&pageSize=${paginationSize}&${searchParams}">${i}</a></li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
	  
	  	 <% if (page.hasNext()){%>
               	<li><a href="${action}?pageNumber=${current+1}&pageSize=${paginationSize}&${searchParams}">&raquo;</a></li>
                <li><a href="${action}?pageNumber=${page.totalPages}&pageSize=${paginationSize}&${searchParams}">尾页</a></li>
         <%}else{%>
