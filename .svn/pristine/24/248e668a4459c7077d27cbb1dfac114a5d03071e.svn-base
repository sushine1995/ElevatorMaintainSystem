		...
		<div class="box">
			...
			<div class="box-body">
				<div class="row">
					<div class="col-md-12">
						<div class="box box-primary">
							...
							<div class="box-body">
								<!-- form start -->
								<form id="searchForm" action="maintainItem/search" method="get">
									<div class="box-body">
										<div class="row">
											<input hidden="true" name="pageNumber" id="pageNumber">
											<div class="form-group col-md-2">
												<label for="nameLabel">项目名:</label> <input type="text"
													class="form-control" id="nameLabel"
													name="search_name" value="${searchParamsMap.name }">
											</div>
											<div class="form-group col-md-2">
												<label for="maintainTypeNameLabel">项目类型:</label> <input type="text"
													class="form-control" id="maintainTypeNameLabel"
													name="search_maintainTypeName" value="${searchParamsMap.maintainTypeName }">
											</div>
				...
				<%-- 		<shiro:hasPermission name="maintainItem:upload">
							...
							<shiro:hasPermission name="maintainItem:download">
							<form id="downloadForm" action="maintainItem/download" method="get" >
				...
				<div class="table-responsive">
					<table class="table table-hover center">
						...
						<c:forEach items="${maintainItems}" var="maintainItem" varStatus="status">
							<tr>
								<td><label><input type="checkbox"
										class="minimal deleteCheckbox" value="${maintainItem.id}"></label></td>
								<td>${status.count}</td>
								...
								<td><shiro:hasPermission name="maintainItem:update">
										<button id="updateBtn" type="button"
											class="btn btn-xs btn-primary btn-flat" onclick='updateItem(${maintainItem.id})'>编辑</button>
									</shiro:hasPermission> <shiro:hasPermission name="maintainItem:view">
										<button id="detailBtn" type="button"
											class="btn  btn-xs btn-primary btn-flat"  onclick='detailItem(${maintainItem.id})'>详情</button>
									</shiro:hasPermission> <shiro:hasPermission name="maintainItem:bind">
										<button id="bindRoleBtn" type="button"
											class="btn  btn-xs btn-primary btn-flat"  onclick='bindItem(${maintainItem.id})'>小组绑定</button>
				...
				<vino:pagination paginationSize="10" page="${page}"
					action="maintainItem/search" contentSelector="#content-wrapper"></vino:pagination>
			</div>
			<!-- /.box -->
		</div>
	</div>
...
<script>
	...
	$('#deleteConfirmModal').on('shown.bs.modal', function(event) {
		$('#deleteConfirmBtn').click(function(){
			deleteItemsUseModal("input[class*='deleteCheckbox']","maintainItem/delete");
		});
	});
	/* button监听事件 */
	$(document).ready(function(){
	...
	$("#searchBtn").click(function() {
		...
		$.ajax({
			...
			url : "maintainItem/search",//请求的action路径  
	...
	function addItem(){
		modalLoadAndDisplay('maintainItem/prepareAdd');
	}
	
	function updateItem(id){	
		modalLoadAndDisplay('maintainItem/'+id);
	}
	
	function detailItem(id){
	
		modalLoadAndDisplay('maintainItem/detail/'+id);
	}
	
	function uploadItem(){	
		modalLoadAndDisplay('maintainItem/prepareUpload');
	}
	function bindItem(id){
		modalLoadAndDisplay('maintainItem/prepareBind/'+id);
