			...
			<div class="box-body">
				<div class="row">
					<div class="col-md-12">
						<div class="box box-primary">
							...
							<div class="box-body">
								<!-- form start -->
								<form id="searchForm" action="maintainType/search" method="get">
									<div class="box-body">
										<div class="row">
											<input hidden="true" name="pageNumber" id="pageNumber">
											<div class="form-group col-md-2">
												<label for="nameLabel">类型名:</label> <input type="text"
				...
				<div class="box box-primary">
					...
					<%-- 	<shiro:hasPermission name="maintainType:upload">
							...
							<shiro:hasPermission name="maintainType:download">
							<form id="downloadForm" action="maintainType/download" method="get" >
				...
				<div class="table-responsive">
					<table class="table table-hover center">
						...
						<c:forEach items="${maintainTypes}" var="maintainType" varStatus="status">
							<tr>
								<td><label><input type="checkbox"
										class="minimal deleteCheckbox" value="${maintainType.id}"></label></td>
								<td>${status.count}</td>
								...
								<td><shiro:hasPermission name="maintainType:update">
										<button id="updateBtn" type="button"
											class="btn btn-xs btn-primary btn-flat" onclick='updateItem(${maintainType.id})'>编辑</button>
									</shiro:hasPermission> <shiro:hasPermission name="maintainType:view">
										<button id="detailBtn" type="button"
											class="btn  btn-xs btn-primary btn-flat"  onclick='detailItem(${maintainType.id})'>详情</button>
									</shiro:hasPermission> <shiro:hasPermission name="maintainType:bind">
										<button id="bindRoleBtn" type="button"
											class="btn  btn-xs btn-primary btn-flat"  onclick='bindItem(${maintainType.id})'>小组绑定</button>
				...
				<vino:pagination paginationSize="10" page="${page}"
					action="maintainType/search" contentSelector="#content-wrapper"></vino:pagination>
			</div>
			<!-- /.box -->
		</div>
	</div>
...
<script>
	...
	$('#deleteConfirmModal').on('shown.bs.modal', function(event) {
		$('#deleteConfirmBtn').click(function(){
			deleteItemsUseModal("input[class*='deleteCheckbox']","maintainType/delete");
		});
	});
	/* button监听事件 */
	$(document).ready(function(){
	...
	$("#searchBtn").click(function() {
		...
		$.ajax({
			...
			url : "maintainType/search",//请求的action路径  
	...
	function addItem(){
		modalLoadAndDisplay('maintainType/prepareAdd');
	}
	
	function updateItem(id){	
		modalLoadAndDisplay('maintainType/'+id);
	}
	
	function detailItem(id){
	
		modalLoadAndDisplay('maintainType/detail/'+id);
	}
	
	function uploadItem(){	
		modalLoadAndDisplay('maintainType/prepareUpload');
	}
	function bindItem(id){
		modalLoadAndDisplay('maintainType/prepareBind/'+id);
