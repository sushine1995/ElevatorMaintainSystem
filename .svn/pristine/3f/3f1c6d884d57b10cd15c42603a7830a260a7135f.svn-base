...
<section class="content">
								...
								<form id="searchForm" action="employee/search" method="get">
									<div class="box-body">
										<div class="row">
											...
											<div class="form-group col-md-2">
												<label for="aliasLabel">姓名:</label> <input type="text"
	...
	<%-- 					<shiro:hasPermission name="employee:upload">
							...
							<shiro:hasPermission name="employee:download">
							<form id="downloadForm" action="employee/download" method="get" >
						...
						<c:forEach items="${employees}" var="employee" varStatus="status">
							<tr>
								<td><label><input type="checkbox"
										class="minimal deleteCheckbox" value="${employee.id}"></label></td>
								<td>${status.count}</td>
								...
								<td><shiro:hasPermission name="employee:update">
										<button id="updateBtn" type="button"
											class="btn btn-xs btn-primary btn-flat" onclick='updateItem(${employee.id})'>编辑</button>
									</shiro:hasPermission> <shiro:hasPermission name="employee:view">
										<button id="detailBtn" type="button"
											class="btn  btn-xs btn-primary btn-flat"  onclick='detailItem(${employee.id})'>详情</button>
									</shiro:hasPermission> <shiro:hasPermission name="employee:bind">
										<button id="bindRoleBtn" type="button"
											class="btn  btn-xs btn-primary btn-flat"  onclick='bindItem(${employee.id})'>小组绑定</button>
				...
				<vino:pagination paginationSize="10" page="${page}"
					action="employee/search" contentSelector="#content-wrapper"></vino:pagination>
			</div>
			<!-- /.box -->
		</div>
	</div>
...
<script>
	...
	$('#deleteConfirmModal').on('shown.bs.modal', function(event) {
		$('#deleteConfirmBtn').click(function(){
			deleteItemsUseModal("input[class*='deleteCheckbox']","employee/delete");
		});
	});
	/* button监听事件 */
	$(document).ready(function(){
	...
	$("#searchBtn").click(function() {
		...
		$.ajax({
			...
			url : "employee/search",//请求的action路径  
	...
	function addItem(){
		modalLoadAndDisplay('employee/prepareAdd');
	}
	
	function updateItem(id){	
		modalLoadAndDisplay('employee/'+id);
	}
	
	function detailItem(id){
	
		modalLoadAndDisplay('employee/detail/'+id);
	}
	
	function uploadItem(){	
		modalLoadAndDisplay('employee/prepareUpload');
	}
	function bindItem(id){
		modalLoadAndDisplay('employee/prepareBind/'+id);
