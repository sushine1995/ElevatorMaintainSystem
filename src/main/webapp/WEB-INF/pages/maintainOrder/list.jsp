			...
			<div class="box-body">
				<div class="row">
					<div class="col-md-12">
						<div class="box box-primary">
							...
							<div class="box-body">
								<!-- form start -->
								<form id="searchForm" action="maintainOrder/search" method="get">
									<div class="box-body">
										<div class="row">
											<input hidden="true" name="pageNumber" id="pageNumber">
											<div class="form-group col-md-2">
												<label for="employeeNameLabel">员工名:</label> <input
													type="text" class="form-control" id="employeeNameLabel"
													name="search_employeeName"
													value="${searchParamsMap.employeeName }">
											</div>
											<div class="form-group col-md-2">
											...
											<div class="form-group col-md-1">
												<label>是否完成:</label> <select class="form-control"
													name="search_fixed">
													<c:choose>
														<c:when test="${searchParamsMap.finished=='true'}">
											...
											<div class="form-group col-md-2">
												<label for="maintainTypeNameLabel">维保类型:</label> <input
													type="text" class="form-control" id="maintainTypeNameLabel"
													name="search_maintainTypeName"
													value="${searchParamsMap.maintainTypeName }">
											</div>
											...
											<div class="form-group  col-md-4">
												...
												<div class="input-group">
													...
													<input type="text" class="form-control pull-right"
														id="finalTimeRange" name="search_finalTimeRange"
														value="${searchParamsMap.finalTimeRange}">
												</div>
												<!-- /.input group -->
											</div>
				...
				<div class="box box-primary">
					...
					<div class="btn-group">
						...
						<shiro:hasPermission name="maintainOrder:download">
							<form id="downloadForm" action="maintainOrder/download"
								method="get">
					...
					<div class="table-responsive">
						<table class="table table-hover center">
							...
							<c:forEach items="${maintainOrders}" var="maintainOrder"
								varStatus="status">
								<tr>
									<td><label><input type="checkbox"
											class="minimal deleteCheckbox" value="${maintainOrder.id}"></label></td>
									<td>${status.count}</td>
									...
									<td><shiro:hasPermission name="maintainOrder:update">
											<button id="updateBtn" type="button"
												class="btn btn-xs btn-primary btn-flat"
												onclick='updateItem(${maintainOrder.id})'>编辑</button>
										</shiro:hasPermission> 
										<shiro:hasPermission name="maintainOrder:view">
											<button id="detailBtn" type="button"
												class="btn  btn-xs btn-primary btn-flat"
												onclick='detailItem(${maintainOrder.id})'>详情</button>
										</shiro:hasPermission>
										<shiro:hasPermission name="maintainOrder:feedback">
											<button id="feedbackBtn" type="button"
												class="btn  btn-xs btn-primary btn-flat"
												onclick='feedbackOrder(${maintainOrder.id})'>反馈</button>
										</shiro:hasPermission>
				...
				<vino:pagination paginationSize="10" page="${page}"
					action="maintainOrder/search" contentSelector="#content-wrapper"></vino:pagination>
			</div>
			<!-- /.box -->
		</div>
	</div>
...
<script>
	//Date range picker
	$('#finalTimeRange').daterangepicker({		 
	        showDropdowns: true
	});
	...
	$('#deleteConfirmModal').on('shown.bs.modal', function(event) {
		$('#deleteConfirmBtn').click(function(){
			deleteItemsUseModal("input[class*='deleteCheckbox']","maintainOrder/delete");
		});
	});
	/* button监听事件 */
	$(document).ready(function(){
	...
	$("#searchBtn").click(function() {
		...
		$.ajax({
			...
			url : "maintainOrder/search",//请求的action路径  
	...
	function addItem(){
		modalLoadAndDisplay('maintainOrder/prepareAdd');
	}
	
	function updateItem(id){	
		modalLoadAndDisplay('maintainOrder/'+id);
	}
	
	function detailItem(id){
	
		modalLoadAndDisplay('maintainOrder/detail/'+id);
	}
	
	function feedbackOrder(id){
		
		modalLoadAndDisplay('maintainOrder/prepareFeedback/'+id);
	}
